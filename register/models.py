...
class ReferralCode(models.Model):
    ...
    times_used = models.IntegerField(
        default=0, verbose_name='Sudah berapa kali digunakan?')
    competition = models.ForeignKey(
        Competition, on_delete=models.CASCADE, default=None)
...
class Team(models.Model):
    ...
    referral_code = models.CharField(
        verbose_name='Kode Referral', blank=True, null=True)
...
class Member(models.Model):
    ...
    def generate_name_photo_follow_instagram(self, ext):
        return f'{self.role}_{self.name}_Follow IG.{ext}'
    ...
    def upload_photo_follow_instagram(instance, filename):
        ext = filename.split('.')[-1]

        return f'uploads/{instance.team_id.team_name}/{instance.generate_name_photo_follow_instagram(ext)}'
    ...
    def save_follow_instagram_img(self, file):
        ext = file.name.split('.')[-1]

        self.photo_follow_ig.save(
            name=self.generate_name_photo_follow_instagram(ext), content=file)
    ...
    photo_follow_ig = models.ImageField(
        verbose_name='Bukti Follow Instagram ICEE ITB', upload_to=upload_photo_follow_instagram, blank=True, null=True)
